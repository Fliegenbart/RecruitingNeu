import { analyzeApplication, analyzeApplicationWithLLM, clusterApplications, getTriageDemo } from '../src/triage.mjs';
import { getStore, nowISO } from '../src/store.mjs';
import { generateDemoApplication } from '../src/demo.mjs';
...
const listTenantContext=(store)=>{const tenants=Object.values(store.db.tenants); const teams=Object.values(store.db.teams); const jobs=Object.values(store.db.jobs).map(j=>({...j,applicationCount:Object.values(store.db.applications).filter(a=>a.jobId===j.id).length})); return {tenants,teams,jobs};};
const analyzeForJob=async({job,text,useLLM=false}={})=>{const opts={jobFamily:job.family,applicationText:text,rubric:job.rubric||{}}; return useLLM?await analyzeApplicationWithLLM(opts):analyzeApplication(opts);};
const reanalyzeJob=async(store,{tenantId,jobId,useLLM=false}={})=>{const job=store.db.jobs[jobId]; if(!job||job.tenantId!==tenantId) throw new Error('job_not_found'); const apps=Object.values(store.db.applications).filter(a=>a.tenantId===tenantId&&a.jobId===jobId); for(const a of apps){const analysis=await analyzeForJob({job,text:a.text,useLLM}); store.updateApplication(a.id,{analysis});} store.addEvent({tenantId,jobId,action:'job_reanalyzed',payload:{count:apps.length}}); return {count:apps.length};};
const clusterJob=(store,{tenantId,jobId}={})=>{const job=store.db.jobs[jobId]; if(!job||job.tenantId!==tenantId) throw new Error('job_not_found'); const apps=Object.values(store.db.applications).filter(a=>a.tenantId===tenantId&&a.jobId===jobId); const threshold=Number.isFinite(Number(job?.rubric?.clusterThreshold))?Number(job.rubric.clusterThreshold):0.82; const shingleSize=Number.isFinite(Number(job?.rubric?.shingleSize))?Number(job.rubric.shingleSize):3; const input=apps.map(a=>({id:a.id,text:a.text})); const clustered=clusterApplications({applications:input,threshold,shingleSize}); for(const a of apps) store.updateApplication(a.id,{clusterId:null,isClusterRepresentative:false}); const clusters=(clustered.clusters||[]).map(c=>{const cid=`${jobId}_${c.clusterId}`; for(const id of c.items) store.updateApplication(id,{clusterId:cid,isClusterRepresentative:id===c.representativeId}); return {...c,clusterId:cid};}); const summary={computedAt:nowISO(),threshold:clustered.threshold,shingleSize:clustered.shingleSize,clusters}; store.setJobCluster(jobId,summary); store.addEvent({tenantId,jobId,action:'job_clustered',payload:{clusters:clusters.length}}); return summary;};
...
export default async function handler(req,res){
  ...
  if (req.method==='GET' && path==='/api/pilot/context') { const store=await getStore({persist:false}); return send(res,200,{success:true,data:listTenantContext(store)}); }
  if (req.method==='POST' && path==='/api/pilot/tenants') { const store=await getStore({persist:false}); const b=await parseBody(req); return send(res,200,{success:true,data:store.createTenant({name:b?.name})}); }
  if (req.method==='POST' && path==='/api/pilot/teams') { const store=await getStore({persist:false}); const b=await parseBody(req); try { return send(res,200,{success:true,data:store.createTeam({tenantId:b?.tenantId,name:b?.name})}); } catch(e){ return send(res,400,{success:false,error:String(e?.message||e)}); } }
  if (req.method==='POST' && path==='/api/pilot/jobs') { const store=await getStore({persist:false}); const b=await parseBody(req); try { return send(res,200,{success:true,data:store.createJob({tenantId:b?.tenantId,teamId:b?.teamId,title:b?.title,family:b?.family,location:b?.location,rubric:b?.rubric})}); } catch(e){ return send(res,400,{success:false,error:String(e?.message||e)}); } }
  if (req.method==='PATCH' && path.startsWith('/api/pilot/jobs/') && path.endsWith('/rubric')) { const store=await getStore({persist:false}); const b=await parseBody(req); const parts=path.split('/').filter(Boolean); const jobId=parts[3]; try { const job=store.db.jobs[jobId]; if(!job||job.tenantId!==b?.tenantId) return send(res,404,{success:false,error:'job_not_found'}); const updated=store.updateJobRubric(jobId,b?.rubric); if(b?.reanalyze) await reanalyzeJob(store,{tenantId:b.tenantId,jobId,useLLM:false}); if(b?.recluster) clusterJob(store,{tenantId:b.tenantId,jobId}); return send(res,200,{success:true,data:updated}); } catch(e){ return send(res,400,{success:false,error:String(e?.message||e)}); } }
  if (req.method==='POST' && path.startsWith('/api/pilot/jobs/') && path.endsWith('/reanalyze')) { const store=await getStore({persist:false}); const b=await parseBody(req); const parts=path.split('/').filter(Boolean); const jobId=parts[3]; try { return send(res,200,{success:true,data:await reanalyzeJob(store,{tenantId:b?.tenantId,jobId,useLLM:Boolean(b?.useLLM)})}); } catch(e){ return send(res,400,{success:false,error:String(e?.message||e)}); } }
  if (req.method==='POST' && path==='/api/pilot/seed') { const store=await getStore({persist:false}); const b=await parseBody(req); const tenantId=b?.tenantId; const jobId=b?.jobId; const count=clamp(Number(b?.count||200),1,1000); const job=store.db.jobs[jobId]; if(!tenantId||!jobId||!job||job.tenantId!==tenantId) return send(res,400,{success:false,error:'tenantId/jobId ungueltig'}); const created=[]; for(let i=0;i<count;i++){const demo=generateDemoApplication({family:job.family}); const app=store.createApplication({tenantId,jobId,candidateName:demo.candidateName,text:demo.text,source:'xing'}); const analysis=await analyzeForJob({job,text:app.text,useLLM:false}); store.updateApplication(app.id,{analysis}); created.push(app.id);} const cluster=clusterJob(store,{tenantId,jobId}); return send(res,200,{success:true,data:{created:created.length,jobId,cluster}}); }
  if (req.method==='POST' && path==='/api/pilot/cluster') { const store=await getStore({persist:false}); const b=await parseBody(req); try { return send(res,200,{success:true,data:clusterJob(store,{tenantId:b?.tenantId,jobId:b?.jobId})}); } catch(e){ return send(res,400,{success:false,error:String(e?.message||e)}); } }
  if (req.method==='GET' && path==='/api/pilot/applications') { const store=await getStore({persist:false}); const tenantId=url.searchParams.get('tenantId'); const jobId=url.searchParams.get('jobId'); if(!tenantId||!jobId) return send(res,400,{success:false,error:'tenantId und jobId erforderlich'}); const status=url.searchParams.get('status'); const mustHave=url.searchParams.get('mustHave'); const q=(url.searchParams.get('q')||'').toLowerCase().trim(); const collapsed=url.searchParams.get('collapsedClusters')==='1'; const sort=url.searchParams.get('sort')||'submitted_desc'; const page=clamp(Number(url.searchParams.get('page')||1),1,9999); const limit=clamp(Number(url.searchParams.get('limit')||50),5,200); let apps=Object.values(store.db.applications).filter(a=>a.tenantId===tenantId&&a.jobId===jobId); if(status) apps=apps.filter(a=>a.status===status); if(mustHave==='pass') apps=apps.filter(a=>a.analysis?.mustHave?.passed===true); if(mustHave==='fail') apps=apps.filter(a=>a.analysis?.mustHave?.passed===false); if(q) apps=apps.filter(a=>(a.candidateName||'').toLowerCase().includes(q)); if(collapsed) apps=apps.filter(a=>!a.clusterId||a.isClusterRepresentative); if(sort==='overall_desc') apps.sort((a,b)=>(b.analysis?.scores?.overall||0)-(a.analysis?.scores?.overall||0)); else apps.sort((a,b)=>String(b.submittedAt).localeCompare(String(a.submittedAt))); const total=apps.length; const start=(page-1)*limit; const items=apps.slice(start,start+limit).map(a=>({id:a.id,candidateName:a.candidateName,submittedAt:a.submittedAt,status:a.status,source:a.source,tags:a.tags,scores:a.analysis?.scores||null,mustHavePassed:a.analysis?.mustHave?.passed??null,clusterId:a.clusterId,isClusterRepresentative:a.isClusterRepresentative,noteCount:a.notes?.length||0})); const job=store.db.jobs[jobId]; return send(res,200,{success:true,data:{page,limit,total,items,cluster:job?.cluster||null}}); }
  if (path.startsWith('/api/pilot/applications/')) { const store=await getStore({persist:false}); const parts=path.split('/').filter(Boolean); const applicationId=parts[3]; if (req.method==='GET' && parts.length===4) { const tenantId=url.searchParams.get('tenantId'); const app=store.db.applications[applicationId]; if(!app||(tenantId&&app.tenantId!==tenantId)) return send(res,404,{success:false,error:'application_not_found'}); const events=store.db.events.filter(e=>e.applicationId===applicationId).slice(-60); return send(res,200,{success:true,data:{application:app,events}}); } if (req.method==='PATCH' && parts.length===4) { const b=await parseBody(req); const tenantId=b?.tenantId; const app=store.db.applications[applicationId]; if(!app||!tenantId||app.tenantId!==tenantId) return send(res,404,{success:false,error:'application_not_found'}); const before=app.status; const updated=store.updateApplication(applicationId,{status:b?.status,tags:b?.tags}); if(b?.status && before!==b.status) store.addEvent({tenantId,jobId:updated.jobId,applicationId,action:'status_changed',payload:{from:before,to:b.status}}); return send(res,200,{success:true,data:updated}); } if (req.method==='POST' && parts.length===5 && parts[4]==='note') { const b=await parseBody(req); const tenantId=b?.tenantId; const app=store.db.applications[applicationId]; if(!app||!tenantId||app.tenantId!==tenantId) return send(res,404,{success:false,error:'application_not_found'}); return send(res,200,{success:true,data:store.addNote(applicationId,{text:b?.text})}); } if (req.method==='POST' && parts.length===6 && parts[4]==='assessment' && parts[5]==='send') { const b=await parseBody(req); const tenantId=b?.tenantId; const app=store.db.applications[applicationId]; if(!app||!tenantId||app.tenantId!==tenantId) return send(res,404,{success:false,error:'application_not_found'}); const updated=store.updateApplication(applicationId,{assessment:{sentAt:nowISO()}}); store.addEvent({tenantId,jobId:updated.jobId,applicationId,action:'assessment_sent',payload:{}}); return send(res,200,{success:true,data:updated.assessment}); } }
  if (req.method==='GET' && path==='/api/triage/demo') return send(res,200,{success:true,data:getTriageDemo()});
  if (req.method==='POST' && path==='/api/triage/analyze') { const b=await parseBody(req); if(!b?.applicationText) return send(res,400,{success:false,error:'applicationText erforderlich'}); const data=b?.useLLM ? await analyzeApplicationWithLLM({jobFamily:b.jobFamily,applicationText:b.applicationText,rubric:b?.rubric}) : analyzeApplication({jobFamily:b.jobFamily,applicationText:b.applicationText,rubric:b?.rubric}); return send(res,200,{success:true,data}); }
  if (req.method==='POST' && path==='/api/triage/cluster') { const b=await parseBody(req); return send(res,200,{success:true,data:clusterApplications({applications:b?.applications||[],threshold:b?.threshold,shingleSize:b?.shingleSize})}); }
